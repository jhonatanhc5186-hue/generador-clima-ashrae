...
# 1. Función para obtener el nombre de la ubicación
def get_location_name(lat, lon):
    url = f"https://nominatim.openstreetmap.org/reverse?lat={lat}&lon={lon}&format=json"
    headers = {'User-Agent': 'GeneradorClimaASHRAE/1.1'} # Necesario para Nominatim
    try:
        res = requests.get(url, headers=headers).json()
        address = res.get('address', {})
        ciudad = address.get('city', address.get('town', address.get('village', 'Ubicación Desconocida')))
        pais = address.get('country', '')
        return f"{ciudad}, {pais}".strip(", ")
    except:
...
if st.button("Generar Reporte"):
    with st.spinner("Procesando datos y detectando ubicación..."):
        # Obtener nombre de ubicación dinámico
        location_name = get_location_name(lat, lon)
        
        # 2. Obtención de datos NASA
        ...
        # 3. HTML con Orientación Horizontal (Landscape)
        filas = "".join([f"""<tr>
        ...
        <body>
            ...
            <p><strong>Ubicación:</strong> {location_name} | <strong>Latitud:</strong> {lat} | <strong>Longitud:</strong> {lon} | <strong>Elevación:</strong> {alt} m</p>
            <table>
        ...
        st.success("¡Reporte generado horizontalmente!")
        st.download_button("📥 Descargar PDF Horizontal", data=pdf_file, file_name="Reporte_ASHRAE.pdf", mime="application/pdf")
