...
# 1. Función para obtener ubicación real
def get_location_name(lat, lon):
    try:
        ...
        # Prioriza Ciudad, si no, Pueblo, si no, lo que encuentre
        return address.get('city', address.get('town', address.get('village', 'Ubicación')))
    except:
...
if st.button("Generar Reporte Profesional"):
    with st.spinner("Procesando datos y aplicando estándares ASHRAE..."):
        ...
        <body>
            <h2 style="color: #2e75b6; border-bottom: 3px solid #2e75b6; margin:0;">CONDICIONES CLIMÁTICAS MENSUALES DE DISEÑO</h2>
            <p style="margin: 5px 0;"><strong>Ubicación:</strong> {city_name} | <strong>Latitud:</strong> {lat} | <strong>Longitud:</strong> {lon} | <strong>Elevación:</strong> {alt} m</p>
            <table>
                ...
                <tr><th colspan="2" class="h_azul">DB 0.4%</th><th colspan="2" class="h_azul">MCWB 0.4%</th><th colspan="2" class="h_azul">DB 2.0%</th><th colspan="2" class="h_azul">MCWB 2.0%</th><th colspan="2" class="h_naranja">DB 99.6%</th><th colspan="2" class="h_naranja">DB 99.0%</th><th colspan="2" class="h_verde">Δ°C | Δ°F</th></tr>
                {filas}
            </table>
            <p style="font-size: 7px; color: #555; margin-top: 5px;">Generado mediante reanálisis de datos NASA POWER (Año {year}). Procesado metodológicamente para aproximación de condiciones ASHRAE. Altitud nativa de la NASA.</p>
        </body></html>"""
        
        pdf_file = HTML(string=html_content).write_pdf()
        st.success("¡Reporte listo!")
        st.download_button("📥 Descargar Reporte PDF", data=pdf_file, file_name=f"Reporte_{city_name}.pdf", mime="application/pdf")
